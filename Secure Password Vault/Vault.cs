...
public partial class Vault : Form
{
    ...
    private async void saveVaultBtn_Click(object sender, EventArgs e)
    {
        ...
        catch (Exception ex)
        {
            EnableUi();
            _isAnimating = false;
            MessageBox.Show(@"There was an error while saving the vault.", @"Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
    ...
    private async void ImportFileBtn_Click(object? sender, EventArgs e)
    {
        ...
        catch (Exception ex)
        {
            ...
            MessageBox.Show(@"There was an error while opening the file.", @"Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            FileOutputLbl.Text = @"Idle...";
            FileOutputLbl.ForeColor = Color.WhiteSmoke;
            ErrorLogging.ErrorLog(ex);
        }
